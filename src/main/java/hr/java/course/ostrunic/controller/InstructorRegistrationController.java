...
import hr.java.course.ostrunic.genericsi.*;
import hr.java.course.ostrunic.model.Instructor;
import hr.java.course.ostrunic.threads.AddInstructorToAdminThread;
import hr.java.course.ostrunic.threads.SaveInstructorThread;
...
public class InstructorRegistrationController {
    ...
    private void register() {
        if (!validateFields()) {
            ...
            try {
                ...
                Instructor newInstructor = new Instructor(id, firstName, lastName, email, username, encryptedPassword, UserType.INSTRUCTOR);
                //DatabaseUtils.saveSystemUser(firstName, lastName, username, email, encryptedPassword, String.valueOf(UserType.INSTRUCTOR));
                //DatabaseUtils.saveInstructor(id);
                //DatabaseUtils.addInstructorToAdministrator(1L, id);

                SaveSystemUserThread saveSystemUserThread = new SaveSystemUserThread(firstName, lastName, username, email, encryptedPassword, String.valueOf(UserType.INSTRUCTOR));
                executor.execute(saveSystemUserThread);

                SaveInstructorThread saveInstructorThread = new SaveInstructorThread(id);
                executor.execute(saveInstructorThread);

                AddInstructorToAdminThread addInstructorToAdminThread = new AddInstructorToAdminThread(1L, id);
                executor.execute(addInstructorToAdminThread);

                FileUtils.saveUserCredentials(username, encryptedPassword);

                SerializationUtils.serializeObject(newInstructor, "dat/instructors.ser");
    ...
    private static void successfullyRegisteredAlert() {
        ...
        alert.setContentText("You have successfully registered as an instructor.");
        alert.showAndWait();
    }
